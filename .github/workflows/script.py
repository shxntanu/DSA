...
def update_readme(directory):
    ...
    for folder, files in sorted_dict.items():
        ...
        for file_name in files:
            ...
            if folder == 'notes': 
                ...
                file_link = f"- {filename} ([LeetCode](https://leetcode.com/problems/{link_text})\{:target=\"_blank\"\}) ([Solution]({os.path.join(folder, file_name)}))\n"
    ...
    if len(root_files) > 0:
        ...
        for file_name in root_files:
            link_text, filename = convert_text(file_name)
            file_link = f"- {filename} ([LeetCode](https://leetcode.com/problems/{link_text})\{:target=\"_blank\"\}) ([Solution]({file_name}))\n"
            content.append(file_link)
